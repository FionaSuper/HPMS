...
class Patient_upload extends CI_Controller {
	...
	public function do_upload()
	{
		...
		switch (strtolower($partner_code)) {
			case "mass":
				$this->upload_Mass();
				break;
			case "ka":
	...
	public function check_patient($PARTNER_CODE, $POLICY_NO, $PATIENT_NAME, $PATIENT_NO){
	...
	public function upload_Mass(){
		...
		if ($correct_type){
			...
			while(!feof($patient)){
				...
				$GP_COPAY = substr($m_line, 69, 4);
				$SP_COPAY = substr($m_line, 73, 4);
				$START_DATE = substr($m_line, 159, 10);
				$TERM_DATE = substr($m_line, 169, 10);

					
				//Check whether the patient already exists
				$exist_patient = $this->check_patient($PARTNER_CODE, $POLICY_NO, $PATIENT_NAME, $PATIENT_NO);
				...
				while(!feof($patient)){
					...
					if (substr($m_line, 0, 6) == "M|GEN|"){
						...
						if ($real_line){
							...
							$contract = $this->check_contract($PARTNER_CODE, $POLICY_NO);
