...
class Doctor extends Admin	
{
	...
	*/
	...
	public function add_fee($id)
	{
		...
		//Get business partner doctor contract
	...
	*/
	public function add_save()
	{
		...
		$this->form_validation->set_rules('E_NAME', 'English Name', 'trim|required|max_length[80]|callback_validate_eng_name');
		$this->form_validation->set_rules('C_NAME', 'Chinese Name', 'trim|max_length[80]|callback_validate_chi_name');
		...
		if ($this->form_validation->run()) {
			...
			if ($save_doctor > 0 && $save_payment > 0) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = 'Your data has been successfully stored into the database. '.anchor('administrator/doctor/edit/' . $save_doctor, 'Edit Doctor').' or  '.anchor('administrator/doctor', ' Go back to list');
				}
	...
	*/
	...
	public function view_partner(){
		...
		$partner_contract = $this->db->get("partner_contract")->result();
		
		$this->data['specific_partner_contract'] = $partner_contract;
		
		$this->db->where("DR_CODE", $dr_code);
		$this->db->where("PARTNER_CODE", $partner_code);
		$this->db->where("CARD_TYPE", $partner_contract[0]->CARD_TYPE);
	...
	public function view_contract(){
		...
		$this->db->where("DR_CODE", $dr_code);
		$this->db->where("PARTNER_CODE", $partner_code);
		$this->db->where("CARD_TYPE", $contract_no);
	...
	public function view($id)
	{
		...
		$this->db->where("DR_CODE", $id);
		$query3 = $this->db->get("doctor_consult_hour");
		...
		//Find Doctor Partner (For provide select option to view different doctor partner contract)
		...
		if ($query6->num_rows() > 0){
			...
			$this->db->where("PARTNER_CODE", $doctor_partner[0]->PARTNER_CODE);
			$this->db->order_by("STATUS", "ASC");
			$partner_contract = $this->db->get("partner_contract")->result();
			$this->data['partner_contracts'] = $partner_contract;
			
			//First contract details (For find the special fee)
			$this->db->where("PARTNER_CODE", $partner_contract[0]->PARTNER_CODE);
			$this->db->where("CARD_TYPE", $partner_contract[0]->CARD_TYPE);
			$this->data['contract_detail'] = $this->db->get("partner_contract")->row();
		...
		$this->db->select("TYPE1, TYPE2, TYPE3");
		$this->db->where("DR_CODE", $id);
		$doctor_type = $this->db->get("doctor")->row();
		
		$this->data['doctor_type'] = $doctor_type;
		
		
		//Find Special fee
		if ($this->data['has_doctor_partner']){
			$this->db->where("DR_CODE", $id);
			$this->db->where("PARTNER_CODE", $partner_contract[0]->PARTNER_CODE);
			$this->db->where("CARD_TYPE", $partner_contract[0]->CARD_TYPE);

			$query7 = $this->db->get("doctor_special_fee");
