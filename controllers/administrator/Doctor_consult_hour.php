		...
		if ($this->form_validation->run()) {
			...
			if ($save_doctor_consult_hour) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['id'] 	   = $save_doctor_consult_hour;
					$this->data['message'] = 'Your data has been successfully stored into the database. '.anchor('administrator/doctor_consult_hour/edit/' . $save_doctor_consult_hour, 'Edit Doctor Consult Hour').' or  '.anchor('administrator/doctor_consult_hour', ' Go back to list');
				} else {
	...
	*/
	public function edit_save($id)
	{
		...
		if ($this->form_validation->run()) {
			...
			$save_doctor_consult_hour = $this->model_doctor_consult_hour->change($id, $save_data);
			...
			if ($save_doctor_consult_hour) {
				if ($this->input->post('save_type') == 'stay') {
					$this->data['success'] = true;
					$this->data['id'] 	   = $id;
					$this->data['message'] = 'Your data has been successfully updated into the database. '.anchor('administrator/doctor_consult_hour', ' Go back to list');
				} else {
