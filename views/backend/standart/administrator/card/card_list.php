...
function domo(){
   ...
   $('*').bind('keydown', 'Ctrl+a', function assets() {
       window.location.href = BASE_URL + '/administrator/Card/add';
       return false;
   });
               ...
               <div class="box box-widget widget-user-2">
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        ...
                        <a class="btn btn-flat btn-success" title="export Card" href="<?= site_url('administrator/card/export'); ?>"><i class="fa fa-file-excel-o" ></i> Export XLS</a>
                        <?php }) ?>
                        <?php is_allowed('card_export', function(){?>
                        <a class="btn btn-flat btn-success" title="export pdf Card" href="<?= site_url('administrator/card/export_pdf'); ?>"><i class="fa fa-file-pdf-o" ></i> Export PDF</a>
                        <?php }) ?>
                     </div>
                     ...
                     <h5 class="widget-user-desc">List All Card <i class="label bg-yellow"><?= $card_counts; ?>  items</i></h5>
                  </div>
                  ...
                  <table class="table table-bordered table-striped dataTable">
                     ...
                     <?php foreach($cards as $card): ?>
                        <tr>
                           <td width="5">
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $card->AUTO_NO; ?>">
                           </td>
                           
                           <td><?= _ent($card->AUTO_NO); ?></td> 
                           <td><?= _ent($card->PARTNER_CODE); ?></td> 
                           <td><?= _ent($card->PARTNER_CONTRACT_NO); ?></td> 
                           <td><?= _ent($card->CARD_TYPE_CODE); ?></td> 
                           <td><?= _ent($card->CARD_TYPE); ?></td> 
                           <td width="200">
                              <?php is_allowed('card_view', function($card) use ($card){?>
                              <a href="<?= site_url('administrator/card/view/' . $card->AUTO_NO); ?>" class="label-default"><i class="fa fa-newspaper-o"></i> View
                              <?php }) ?>
                              <?php is_allowed('card_update', function($card) use ($card){?>
                              <a href="<?= site_url('administrator/card/edit/' . $card->AUTO_NO); ?>" class="label-default"><i class="fa fa-edit "></i> Update</a>
                              <?php }) ?>
                              <?php is_allowed('card_delete', function($card) use ($card){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('administrator/card/delete/' . $card->AUTO_NO); ?>" class="label-default remove-data"><i class="fa fa-close"></i> Remove</a>
                               <?php }) ?>
                           </td>
                        </tr>
                      <?php endforeach; ?>
                  ...
                  <div class="col-md-8">
                     ...
                     <div class="col-sm-3 padd-left-0 " >
                        <select type="text" class="form-control chosen chosen-select" name="f" id="field" >
                           ...
                           <option <?= $this->input->get('f') == 'PARTNER_CONTRACT_NO' ? 'selected' :''; ?> value="PARTNER_CONTRACT_NO">PARTNER CONTRACT NO</option>
                           <option <?= $this->input->get('f') == 'CARD_TYPE_CODE' ? 'selected' :''; ?> value="CARD_TYPE_CODE">CARD TYPE CODE</option>
                           <option <?= $this->input->get('f') == 'CARD_TYPE' ? 'selected' :''; ?> value="CARD_TYPE">CARD TYPE</option>
                          </select>
                     </div>
